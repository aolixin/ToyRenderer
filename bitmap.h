...
{
public:
	inline virtual ~Bitmap() { delete[]_bits; _bits = nullptr; }
	...
	inline explicit Bitmap(const char* filename) {
...
public:
	...
	inline static Bitmap* LoadFile(const char* filename) {
		FILE* fp;
		auto openFileErr = fopen_s(&fp, filename, "rb");
		if (openFileErr != 0)
		{
		...
		if (hr != 14) { fclose(fp); return nullptr; }
		if (header[0] != 0x42 || header[1] != 0x4d) { fclose(fp); return nullptr; }
		hr = (int)fread(&info, 1, sizeof(info), fp);
		if (hr != 40) { fclose(fp); return nullptr; }
		if (info.biBitCount != 24 && info.biBitCount != 32) { fclose(fp); return nullptr; }
	...
	inline bool SaveFile(const char* filename, bool withAlpha = false) const {
		FILE* fp;
		auto openFileErr = fopen_s(&fp, filename, "wb");
		if (openFileErr != 0)
		{
			std::cout << "File opened failed" << std::endl;
		}
