...
struct TGAImage
{
	...
	inline std::uint32_t get_uint32_t(const int x, const int y) const;
...
std::uint32_t TGAImage::get_uint32_t(const int x, const int y) const
{
	TGAColor ret = get(x, y);
	return ret.bgra[3] << 24 | ret.bgra[2] << 16 | ret.bgra[1] << 8 | ret.bgra[0];
}
...
TGAColor TGAImage::sample2D(Vec2f& uvf)
{
	return get(uvf[0] * width(), uvf[1] * height());
}
